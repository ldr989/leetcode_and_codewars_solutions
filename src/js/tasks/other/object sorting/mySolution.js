...
const byPriceBtn = document.querySelector(".price");
const byFeed = document.querySelector(".feed");
...
function buildCorrectArray(array) {
    array.forEach((item) => {
...
const correctArray = buildCorrectArray(data);

function sort(array, price = false) {
    let key;
    if (price) {
        key = "price";
    } else {
    ...
    return array.sort(function (a, b) {
        return +b[key].match(/\d+/g).join("") - +a[key].match(/\d+/g).join("");
    });
}

function buildResult(div, arr, price = false) {
    const result = sort(arr, price);

    div.innerHTML = "";

    result.forEach((item, i) => {
        const itemCard = document.createElement("div");
        itemCard.textContent = `${i + 1}. ${item.name}\n цена: ${
            item.price
        }\n Колличество отзывов: ${item.ratingRevievs}`;
        div.appendChild(itemCard);
    });
}

byPriceBtn.addEventListener("click", () => {
    buildResult(resultDiv, correctArray, true);
});

byFeed.addEventListener("click", () => {
    buildResult(resultDiv, correctArray);
});
