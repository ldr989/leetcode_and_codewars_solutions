...
function getList(roles, text) {
    const splittedText = text.split("\n");
    let list = "";
    // function removeSymbol(string, length, symbol) {
    //     if (string.slice(length + 1).startsWith(symbol)) {
    //         return string.slice(length + 2);
    //     } else {
    ...
    roles.forEach((character) => {
        ...
        splittedText.forEach((string, i) => {
            if (string.startsWith(character)) {
                ...
                // )}`;
                characterReplicas += `\n${i + 1}) ${string
                    .slice(character.length + 1)
                    .trim()}`;
            }
        });
        list += `\n${character}:${characterReplicas}\n`;
    });
    return list;
}

console.log(getList(roles, text));
